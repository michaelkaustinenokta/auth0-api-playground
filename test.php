...
function curlToPhp($curlCode) {
    ...
    */
    ...
    foreach($px as $p) {
        $pxt = str_replace(array("\r", "\n", "  ", " \\"), '', $p);
        
        if(preg_match("/--location/", $pxt)){
            $pxps = preg_split("/--location '/",$pxt);

            $pxps = preg_replace("/\'$/","",$pxps[1]);
            
            $pxa["location"] = $pxps;

        }
        if(preg_match("/--header/", $pxt)){
            $pxps = preg_split("/--header '/",$pxt);

            $pxps = preg_replace("/\'$/","",$pxps[1]);

            array_push($pxa["headers"],$pxps);

        }

        if(preg_match("/--data-urlencode '/", $pxt) || preg_match("/--data '/", $pxt) || preg_match("/--data-raw '/", $pxt)){
            $pxps = preg_split("/--data-urlencode '/",$pxt);

            $pxps = preg_replace("/\'$/","",$pxps[1]);

            array_push($pxa["data"],str_replace("<dashdash>","--",$pxps));
